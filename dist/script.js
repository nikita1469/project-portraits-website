...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_scrolling__WEBPACK_IMPORTED_MODULE_10__["default"])('.pageup'); // smooth scroll
...
var accordion = function accordion(trigsSelector) {
  var btns = document.querySelectorAll(trigsSelector);
...
var drop = function drop() {
  ...
  function highlight(item) {
    item.closest('.file_upload').style.borderRadius = '30px 30px';
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      input.files = e.dataTransfer.files; // объкет с файлом, который перетаскиваем из файловой структуры
...
var forms = function forms() {
  ...
  var message = {
    loading: 'Загрузка...',
    success: 'СпасибоБ с вами скоро свяжутся',
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      if (!btnPressed && window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
        document.querySelector(selector).click(); // как-будто мы кликнули по элементу
      }
    });
  }
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResource"])('http://localhost:3000/styles') // path to json
    .then(function (res) {
